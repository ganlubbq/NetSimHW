function [ avg_total_dl, rho_est, i, renewal_instant ] = MG1queue_func( rho, number_of_events, number_of_desired_renewals )
% Function that simulates a DE queue.
slot_len = 1; % in seconds
interarr_rate = rho/1.5*slot_len; % in seconds
b = 0.5;
...
inter_arr = getInterarrivalTime('exp', interarr_rate);
next_arr = inter_arr;
...
while (i  <= number_of_events && renewal_instant <= number_of_desired_renewals)
    ...
    if next_arr <= next_dep %arrival
        clock = next_arr;
        next_arr = clock + getInterarrivalTime('exp', interarr_rate);
